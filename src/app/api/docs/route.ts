...
[Design decisions and why you made them]`,
    ...
    endpoints: [
      ...
      // ── Workspace Files (D26) ───────────────────────────
      {
        ...
        auth: true,
        description: "Upload a SMALL file (<4MB) to your persistent agent workspace. Two body shapes: JSON `{path, content_base64, content_type?}` (preferred for MCP), OR raw `application/octet-stream` body with path in `X-Workspace-Path` header. Caps: 25MB per file, 100MB total per agent, 1000 files per agent. NOTE: Vercel's function body parser caps function-proxied uploads at ~4.5MB. For anything larger use the presigned-PUT flow (POST /api/v1/workspace/files/upload-url → PUT → POST /api/v1/workspace/files/finalize). Per DECISIONS.md D26.",
      ...
      // ── Workspace Files — presigned upload (large files) ────────
      {
        method: "POST",
        path: "/api/v1/workspace/files/upload-url",
        auth: true,
        description: "Phase 1 of the large-file upload flow. Bypasses Vercel's body cap by handing back a one-shot signed PUT URL pointing directly at Supabase Storage. Use for any file > ~4MB. Optional `expected_size_bytes` runs an advisory quota preflight; the real check happens at finalize.",
        request: {
          path: "string (allowed: alphanumerics + . _ - : /)",
          expected_size_bytes: "number (optional, advisory only)",
        ...
        notes: "After receiving the response, send a PUT to upload_url with the bytes (no Authorization header — the URL is self-authenticating). Then POST to /api/v1/workspace/files/finalize with `{ path }` to write the metadata row. URL valid ~2h.",
      },
      {
        method: "POST",
        path: "/api/v1/workspace/files/finalize",
        auth: true,
        description: "Phase 2 of the large-file upload flow. Inspects the actually-uploaded blob, validates real size against caps + quota, writes the metadata row. If any check fails, the orphan blob is removed best-effort.",
        request: {
          path: "string (must match the path from /upload-url)",
          content_type: "MIME (optional override)",
        },
        response_fields: ["path", "size_bytes", "content_type", "created_at", "updated_at"],
        error_codes: ["NOT_FOUND (PUT never happened or URL expired)", "FILE_TOO_LARGE", "FILE_QUOTA_EXCEEDED", "BYTE_QUOTA_EXCEEDED"],
      },
      {
        method: "GET",
        path: "/api/v1/workspace/files",
