...
- [x] Verify submission quota (D15) — **bug found and fixed 2026-05-06**. Constants, validation, MCP descriptions, /api/docs, and public docs all said "default 15, hard cap 25" — but migration 019 set `tasks.max_submissions_per_agent NOT NULL DEFAULT 5`. Because the column is NOT NULL, the in-app `?? TASK_DEFAULT_SUBMISSION_QUOTA` fallback was dead code; every task created without specifying the field got quota 5, not 15. Fix: migration `038_quota_default_align.sql` sets `DEFAULT 15` to match the contract. Existing rows untouched. The CHECK cap (1..100, set in migration 029) stays — Zod in `validation.ts` already caps user input at 25, so the wider DB ceiling is intentional headroom. Lesson captured in `tasks/lessons.md`. **Pending: `supabase db push` from Jeremy** to apply 038 to remote.
...
- [x] **Binary-safe file uploads** (2026-05-06). Backwards-compatible widening of `quick-submit` `files` field: each value can now be a string (UTF-8 text — legacy unchanged) OR `{ content, encoding?: "utf8" | "base64", contentType?: string }`. Stack: new helper `src/lib/submission-files.ts` (decoded once at the route boundary, returns `{ buffer, contentType }` per file with extension-based MIME sniff and strict base64 validation), updated `validateSubmissionAgainstContract` to operate on `Record<string, Buffer>` (size accounting now uses decoded byte length, accurate for binary), updated route `src/app/api/v1/tasks/[id]/quick-submit/route.ts` to decode early + upload with the correct contentType (no more force-`text/plain`). Updated SDK type `SubmissionFileEntry` + `QuickSubmitOptions.files`. Updated MCP `quick_submit` tool input schema (Zod union) + description warns about silent corruption when sending binaries as strings. New test file `src/lib/submission-files.test.ts` (15 tests covering MIME sniff, base64 decode, mixed maps, error collection, byte-length correctness). Existing `submission-contract.test.ts` updated to wrap fixtures in `Buffer`s. Stacked into the un-published `@strawai/agent-sdk@0.3.0` / `@strawai/mcp-server@1.3.0`. Total test count 905 → 920.

**P2 — annoyances with clean workarounds**
- [x] Surface `quota_remaining` everywhere + new `check_quota` MCP tool (2026-05-06). `get_task` already returned quota — that part was a no-op verification. Real fixes: (1) `POST /api/v1/tasks/[id]/quick-submit` response now includes `quota: { used, limit, remaining }` computed AFTER the just-created submission, so an agent knows headroom in one round-trip instead of polling get_task. (2) New SDK method `client.tasks.checkQuota(taskId)` wraps `get` and projects `{ task_id, quota }` — explicit error when quota is null (caller is the task owner, not an agent). (3) New MCP tool `check_quota` invokes it; description tells the agent "doesn't consume a quota slot" + warns when remaining ≤ 2. (4) `formatQuickSubmitResult` formatter prints the new quota line. Bumped `@strawai/agent-sdk` 0.2.0 → **0.3.0** and `@strawai/mcp-server` 1.2.0 → **1.3.0**, plus root `package.json` to consume both. **Pending: Jeremy `npm publish` both packages** (cd packages/agent-sdk && npm publish, then cd ../mcp-server && npm install && npm publish). Local mcp-server type-check works because I mirrored the freshly-built agent-sdk dist into `node_modules/@strawai/agent-sdk/` — that mirror is fragile (gets clobbered on next `npm install`), so publish-then-install before any other dependency churn.
- [x] Ship `straw eval --local` (2026-05-06). Stack: (1) new `src/services/eval-preview.service.ts` with self-contained prompt builder + Gemini caller (intentionally subset of the worker — single-pass LLM-only, no test_weight, no container, no multi-pass — drift accepted to avoid coupling the route to the worker process). Tier-1 only; preview disclosure text spells this out so agents know real scores can differ. (2) New route `POST /api/v1/eval/preview` — auth required, rate-limited at 10/hour/user, 5MB payload cap, never persists anything (no submission row, no evaluation_result, no quota consumed). (3) New SDK `EvalResource` exposed as `client.eval.preview(taskId, files)`. (4) New MCP tool `preview_eval` (warns "no quota used", "real scores may differ"). (5) New CLI binary `straw-eval` in `packages/mcp-server` — walks a directory, encodes binaries as base64, calls the SDK, prints per-criterion breakdown. Added second bin entry to mcp-server package.json + tsup config. (6) New eslint exception: `bin/**` exempt from the no-stdout rule (CLIs own stdout — the rule's purpose was protecting MCP stdio protocol, which CLI bins aren't part of). (7) 7 new unit tests for the preview prompt builder (covers SUBMISSION.md ordering, binary placeholder, security rule presence, prompt-injection marker stripping). 920 → 927 tests pass. Stacked into the un-published `@strawai/agent-sdk@0.3.0` / `@strawai/mcp-server@1.3.0` — Jeremy publishes both together.
...
- [x] ESLint rule banning `console.log` in `packages/mcp-server` (2026-05-06). Scoped override in root `eslint.config.mjs` with `no-console: ["error", { allow: ["error", "warn"] }]` for `packages/mcp-server/src/**/*.ts`. New helper `packages/mcp-server/src/lib/log.ts` exports `strawLog`/`strawWarn`/`strawError` (all routed through `console.error`/`console.warn` → stderr). Verified rule fires: probed with a temp `console.log`, eslint exited 1 with "Unexpected console statement" — probe removed. Existing source is already clean (only `console.error` in bin/straw-mcp.ts), so the rule is preventive, not retroactive. No package.json version bump needed — log.ts isn't reachable from the published entries (tsup ships only index.ts + bin), so it's a source-side dev helper.
- [ ] Collapse MCP tool surface from 8 → ~4 — **designed, deferred 2026-05-06**. Full design + migration plan written up at `tasks/proposals/mcp-tool-surface-collapse.md`. Recommendation in proposal: HOLD. Reasoning: B + C just stacked into 1.3.0, three rapid mcp-server publishes in one week (1.1 → 1.2 → 1.3 → 2.0) is churn for early adopters, the "tool catalog crowding" pain is hypothetical (no agent feedback yet), and D's value is a refinement vs A/B/C which closed real gaps. Revisit when agent feedback indicates crowding is real, or bundle into the next major breaking change. Code unchanged in this batch.
...
- [x] **Missing `notification_preferences` table** — **fixed 2026-05-06**. Root cause was bigger than expected: migration 026 had been added to `schema_migrations` by the 2026-05-06 backfill but its DDL never actually ran on the live DB. So FOUR things were silently broken in prod: (1) `notification_preferences` table missing → PGRST205 every notification dispatch, (2) `notifications.dismissed_at` column missing → dismiss feature dead, (3) `webhook_deliveries` had OLD column names (`event`/`response_code`/`delivered_at`) while code wrote NEW names (`event_type`/`response_status`/`completed_at`) → every webhook delivery write failing, (4) `task_invitations.company_id` missing. Fix: applied all four sections of 026 via `mcp__supabase__execute_sql` (idempotent — used `IF NOT EXISTS` / `IF EXISTS` gates throughout). Verified all six post-conditions returning true. Live DB now matches migrations 026 + 038. Spot-checked 028/029/032/033/037 — all landed correctly, only 026 was the gap. Webhook deliveries table is empty (0 rows) so the rename was risk-free; notifications has 35 rows but only had a column added.
...
- [x] **Delete the stale `e2e-pipeline.ts` script** + update TESTING.md (2026-05-06). Removed `scripts/e2e-pipeline.ts`, removed `"test:pipeline"` from `package.json`, rewrote `tasks/TESTING.md` to point at the current `/api/dev/pipeline-test` endpoint and `/dev/pipeline` GUI (the post-Phase-17 path). The new TESTING.md documents both the synthetic harness and the real agent-submission path (SDK / MCP `quick_submit`), the dev-endpoint double-gate (`NODE_ENV=development` + `ALLOW_DEV_ENDPOINTS=true`), and Upstash vs local Redis. The `scripts/`, `test-agents/`, and `straw-test/*` Docker references are all gone.
- [x] **Sweep code-side hardcoded `tasks/X.md` paths** (2026-05-06). Replaced 11 hardcoded paths across 9 files with wikilink slugs or decision IDs: `src/app/docs/page.tsx` (user-facing — was rendering `<code>tasks/AGENT_FIRST_DREAM.md</code>`, now generic prose), `src/services/workspace.service.ts`, `src/services/search.service.ts`, `src/constants.ts`, `src/__tests__/invariants/cross-agent-isolation.test.ts`, `.env.prod.example`, `DEPLOY.md` (2 sites), `supabase/migrations/027_performance_indexes.sql`, `supabase/migrations/032_agent_workspace_kv.sql` (3 sites including the `COMMENT ON TABLE` SQL — the live DB comment is now stale-but-harmless rot, not worth a migration). All edits use the wikilink form `[[scale]]`/`[[scale-pass-plan]]`/`[[service-role-audit]]`/`[[overnight-log]]`/`[[agent-first-dream]]` per CLAUDE.md's reference rule. CLAUDE.md itself + doc-to-doc cross-references inside `tasks/` were left alone (anchor-list allows the former; the latter is intentional). Lint + 905-test suite both green.
